...
@Configuration
@MapperScan(basePackages = {"com.mallcai.fulfillment.infrastructure.mapper.tms"},sqlSessionFactoryRef="ftmsSqlSessionFactory")
public class DataSourceConfig {

    @Value("${ftms.db.url}")
    private String dbUrl;

    @Value("${ftms.db.username}")
    private String dbUserName;

    @Value("${ftms.db.password}")
    ...
    @Bean(name = "ftmsDataSource",initMethod = "init",destroyMethod = "close")
    @Primary
    ...
    @Bean(name = "ftmsSqlSessionFactory")
    @Primary
    public SqlSessionFactory sqlSessionFactory(@Qualifier("ftmsDataSource") DataSource datasource)
            throws Exception {
        SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
        PathMatchingResourcePatternResolver pathResolver = new PathMatchingResourcePatternResolver();
        bean.setDataSource(datasource);
        bean.setMapperLocations(pathResolver.getResources("classpath*:mybatis/tms/*.xml"));
        return bean.getObject();
    }
    @Bean("ftmsSqlSessionTemplate")
    @Primary
    public SqlSessionTemplate sqlsessiontemplate(
            @Qualifier("ftmsSqlSessionFactory") SqlSessionFactory sessionfactory) {
