...
@Configuration
@MapperScan(basePackages = {"com.mallcai.fulfillment.infrastructure.mapper.wms"},sqlSessionFactoryRef="wmsSqlSessionFactory")
public class WmsDataSourceConfig {

    @Value("${wms.db.url}")
    private String dbUrl;

    @Value("${wms.db.username}")
    private String dbUserName;

    @Value("${wms.db.password}")
    ...
    @Bean(name = "wmsDataSource",initMethod = "init",destroyMethod = "close")
    ...
    public SqlSessionFactory test1SqlSessionFactory(@Qualifier("wmsDataSource") DataSource datasource)
            throws Exception {
        SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
        PathMatchingResourcePatternResolver pathResolver = new PathMatchingResourcePatternResolver();
        bean.setDataSource(datasource);
        bean.setMapperLocations(pathResolver.getResources("classpath*:mybatis/mapper/wms/*.xml"));
        return bean.getObject();
    }
    @Bean("wmsSqlSessionTemplate")
    public SqlSessionTemplate test1sqlsessiontemplate(
            @Qualifier("wmsSqlSessionFactory") SqlSessionFactory sessionfactory) {
