...
@Configuration
@MapperScan(basePackages = {"com.mallcai.fulfillment.infrastructure.mapper.pe"},sqlSessionFactoryRef="fpeSqlSessionFactory")
public class PeDataSourceConfig {
    ...
    @Bean(name = "fpeDataSource",initMethod = "init",destroyMethod = "close")
    ...
    @Bean(name = "fpeSqlSessionFactory")
    public SqlSessionFactory test1SqlSessionFactory(@Qualifier("fpeDataSource") DataSource datasource)
            throws Exception {
        SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
        PathMatchingResourcePatternResolver pathResolver = new PathMatchingResourcePatternResolver();
        bean.setDataSource(datasource);
        bean.setMapperLocations(pathResolver.getResources("classpath*:mybatis/mapper/pe/*.xml"));
        return bean.getObject();
    }
    @Bean("fpeSqlSessionTemplate")
    public SqlSessionTemplate test1sqlsessiontemplate(
            @Qualifier("fpeSqlSessionFactory") SqlSessionFactory sessionfactory) {
