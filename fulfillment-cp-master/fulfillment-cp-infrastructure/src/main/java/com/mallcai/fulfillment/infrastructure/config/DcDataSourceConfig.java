...
@Configuration
@MapperScan(basePackages = {"com.mallcai.fulfillment.infrastructure.mapper.dc"},sqlSessionFactoryRef="fdcSqlSessionFactory")
public class DcDataSourceConfig {

    @Value("${fdc.db.url}")
    private String dbUrl;

    @Value("${fdc.db.username}")
    private String dbUserName;

    @Value("${fdc.db.password}")
    ...
    @Bean(name = "fdcDataSource",initMethod = "init",destroyMethod = "close")
    ...
    @Bean(name = "fdcSqlSessionFactory")
    public SqlSessionFactory test1SqlSessionFactory(@Qualifier("fdcDataSource") DataSource datasource)
            throws Exception {
        SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
        PathMatchingResourcePatternResolver pathResolver = new PathMatchingResourcePatternResolver();
        bean.setDataSource(datasource);
        bean.setMapperLocations(pathResolver.getResources("classpath*:mybatis/mapper/dc/*.xml"));
        return bean.getObject();
    }
    @Bean("fdcSqlSessionTemplate")
    public SqlSessionTemplate test1sqlsessiontemplate(
            @Qualifier("fdcSqlSessionFactory") SqlSessionFactory sessionfactory) {
