...
@Configuration
@MapperScan(basePackages = {"com.mallcai.fulfillment.infrastructure.mapper.big"},sqlSessionFactoryRef="bigSqlSessionFactory")
public class BigDataSourceConfig {

    @Value("${big.db.url}")
    private String dbUrl;

    @Value("${big.db.username}")
    private String dbUserName;

    @Value("${big.db.password}")
    ...
    @Bean(name = "bigDataSource",initMethod = "init",destroyMethod = "close")
    ...
    @Bean(name = "bigSqlSessionFactory")
    public SqlSessionFactory test1SqlSessionFactory(@Qualifier("bigDataSource") DataSource datasource)
            throws Exception {
        SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
        PathMatchingResourcePatternResolver pathResolver = new PathMatchingResourcePatternResolver();
        bean.setDataSource(datasource);
        bean.setMapperLocations(pathResolver.getResources("classpath*:mybatis/mapper/big/*.xml"));
        return bean.getObject();
    }
    @Bean("bigSqlSessionTemplate")
    public SqlSessionTemplate test1sqlsessiontemplate(
            @Qualifier("bigSqlSessionFactory") SqlSessionFactory sessionfactory) {
